(function () {
    ...
    class CacheDatabase {
        ...
        'delete'(name) {
            ...
            return this.scope.caches.delete(`ngsw:staff:db:${name}`);
        }
        list() {
            return this.scope.caches.keys().then(keys => keys.filter(key => key.startsWith('ngsw:staff:db:')));
        }
        open(name) {
            if (!this.tables.has(name)) {
                const table = this.scope.caches.open(`ngsw:staff:db:${name}`)
    ...
    class AppVersion {
        constructor(scope, adapter, database, idle, manifest, manifestHash) {
            ...
            this.assetGroups = (manifest.assetGroups || []).map(config => {
                ...
                // group.
                const prefix = `ngsw:staff:${this.manifestHash}:assets`;
            ...
            this.dataGroups = (manifest.dataGroups || [])
                .map(config => new DataGroup(this.scope, this.adapter, config, this.database, `ngsw:staff:${config.version}:data`));
    ...
    class Driver {
        ...
        deleteAllCaches() {
            return __awaiter$5(this, void 0, void 0, function* () {
                yield (yield this.scope.caches.keys())
                    .filter(key => key.startsWith('ngsw:staff:'))
        ...
        cleanupOldSwCaches() {
            return __awaiter$5(this, void 0, void 0, function* () {
                ...
                const oldSwCacheNames = cacheNames.filter(name => /^ngsw:staff:(?:active|staged|manifest:.+)$/.test(name));
